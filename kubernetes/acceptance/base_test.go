...
import (
	...
	"github.com/chnsz/terraform-provider-kubernetes/kubernetes"
	"github.com/chnsz/terraform-provider-kubernetes/util"
)

// Global constants for testing images (reduces the number of docker pulls).
const (
	busyboxImage = "busybox:1.36"
	alpineImage  = "alpine:3.19.0"
...
func TestProvider_impl(t *testing.T) {
	var _ schema.Provider = *kubernetes.Provider()
}
...
func testAccPreCheck(t *testing.T) {
	...
	hasAkSk := os.Getenv("KUBE_HOST") != "" && os.Getenv("HW_ACCESS_KEY") != "" && os.Getenv("HW_SECRET_KEY") != "" && os.Getenv("HW_PROJECT_ID") != ""

	if !hasFileCfg && !hasStaticCfg && !hasUserCredentials && !hasAkSk {
...
func getClusterVersion() (*gversion.Version, error) {
	...
	conn, err := meta.(baseprovider.KubeClientsets).MainClientset()
	if err != nil {
		return nil, err
	}
...
func useAdmissionregistrationV1beta1(conn *k8sClient.Clientset) (bool, error) {
	...
	if err == nil {
		...
		useadmissionregistrationv1beta1 = util.BoolPtr(false)
		return false, nil
	}
	...
	useadmissionregistrationv1beta1 = util.BoolPtr(true)
	return true, nil
}
